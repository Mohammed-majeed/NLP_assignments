...
# Problem 7 Q&A
""" Compare the generated text quality to each of the models.
    The uni-gram model shows the worst performance, compared to the other two models.
    An example which shows the uni-gram model's weakness is the following:
    "republicans privilege <s> after he felix a </s> " which is not a coherent sentence.
    While the bi-gram and smoothed model start to show some coherence. The model always
    start with the <s> token, and ends with the </s> token. The bi-gram model is able to
    generate coherent sentences, but the smoothed model is able to generate more coherent
    sentences. An example of a sentence generated by the smoothed model is the following:
    "<s> polls to teacher employes similar actions both opened management wasn't" which
    is obviously a not a full coherent sentence and the compositionality isn't correct.
    However, it has some form of compositionality as it has teacher, employes, management
    in the sentence. 
"""